# Unity HTML5 视频播放器

轻量且易用的视频播放管理器，适用于使用 [AVPro Video](https://www.renderheads.com/content/apis/) 的 Unity 项目。可管理多个独立的视频组，并通过 UI 按钮对各组进行播放、暂停和停止控制。

## 特性

✨ **多视频支持** - 同时管理多个视频组  
🎮 **简单的 UI 集成** - 可在 Inspector 中直接绑定按钮  
⚡ **异步加载** - 非阻塞的视频初始化  
🛡️ **健壮的错误处理** - 完整的校验与日志记录  
📋 **良好注释** - 提供完整 XML 文档以支持 IntelliSense  
🎯 **可用于生产** - 包含自动清理与安全检查  

## 要求

- **Unity 2020.3+**（或任何支持 AVPro Video 的版本）
- **AVPro Video 插件** - 必须在项目中安装
- 视频文件应放置于 `StreamingAssets` 文件夹中

## 安装

1. 将 `HTML5VideoPlayer.cs` 复制到项目的 `Assets/Scripts` 文件夹
2. 确保已安装并配置好 AVPro Video 插件
3. 将该脚本附加到场景中的任意 GameObject

## 快速开始

### 基本设置

1. **在场景中创建 GameObject** 并添加 `HTML5VideoPlayer` 组件
2. **在 Inspector 中配置视频组**：
   - 设置所需的视频组数量
   - 对于每个视频组：
     - 指定 **播放按钮**（UI Button）
     - 指定 **暂停按钮**（UI Button）
     - 指定 **停止按钮**（UI Button）
     - 指定 **MediaPlayer**（AVPro Video 的 MediaPlayer 组件）
     - 指定 **DisplayUGUI**（AVPro Video 的 DisplayUGUI 组件）
     - 设置 **视频文件名**（相对于 StreamingAssets 文件夹的路径）

3. **将视频放置在** `Assets/StreamingAssets/` **目录下**

### Inspector 示例配置

```
HTML5VideoPlayer (组件)
├─ 视频组列表
│  ├─ [0]
│  │  ├─ 播放按钮: [UI 按钮引用]
│  │  ├─ 暂停按钮: [UI 按钮引用]
│  │  ├─ 停止按钮: [UI 按钮引用]
│  │  ├─ 媒体播放器: [MediaPlayer 引用]
│  │  ├─ 显示组件: [DisplayUGUI 引用]
│  │  └─ 视频文件名: "myvideo.mp4"
│  └─ [1]
│     └─ [第二个视频的类似配置]
```

## API 参考

### 公开方法

#### `PlayVideo(int groupIndex)`
播放指定视频组的视频。会自动停止其他正在播放的视频以避免重叠。

```csharp
videoPlayer.PlayVideo(0); // 播放第一个视频组
```

#### `PauseVideo(int groupIndex)`
暂停指定视频组的视频。

```csharp
videoPlayer.PauseVideo(0); // 暂停第一个视频组
```

#### `StopVideo(int groupIndex)`
停止视频并将播放位置重置到开头。

```csharp
videoPlayer.StopVideo(0); // 停止第一个视频组
```

#### `GetVideoGroupCount()`
返回已配置的视频组总数。

```csharp
int count = videoPlayer.GetVideoGroupCount();
```

#### `IsVideoGroupLoaded(int groupIndex)`
检查指定的视频组是否已成功加载。

```csharp
if (videoPlayer.IsVideoGroupLoaded(0))
{
    Debug.Log("视频 0 已加载，准备就绪");
}
```

## 高级用法

### 编程控制

```csharp
public class VideoController : MonoBehaviour
{
    [SerializeField] private HTML5VideoPlayer videoPlayer;
    
    public void OnPlayButtonClicked()
    {
        videoPlayer.PlayVideo(0);
    }
    
    public void OnPauseButtonClicked()
    {
        videoPlayer.PauseVideo(0);
    }
    
    public void OnStopButtonClicked()
    {
        videoPlayer.StopVideo(0);
    }
}
```

### 检查视频状态

```csharp
// 检查所有视频是否已加载
for (int i = 0; i < videoPlayer.GetVideoGroupCount(); i++)
{
    if (videoPlayer.IsVideoGroupLoaded(i))
    {
        Debug.Log($"视频 {i} 已加载并准备就绪");
    }
}
```

## 视频文件路径说明

- 视频文件应放置于 `Assets/StreamingAssets/`
- 使用相对路径且不要包含 `StreamingAssets/` 前缀
- 示例：若视频位于 `Assets/StreamingAssets/videos/intro.mp4`，则将文件名设置为 `videos/intro.mp4`
- 支持的格式取决于运行平台和 AVPro Video 的配置

## 故障排查

### 视频未加载

1. **检查控制台日志** - 查找以 `[HTML5VideoPlayer]` 为前缀的错误信息
2. **确认文件路径** - 确保视频文件位于 `StreamingAssets` 文件夹
3. **检查 AVPro 设置** - 确保 AVPro Video 正确配置
4. **验证引用** - 确保 Inspector 中的引用已正确分配（无空引用）

### 常见问题

| 问题 | 解决方法 |
|-------|----------|
| "VideoGroup X 未配置 MediaPlayer" | 在 Inspector 中为该视频组分配 MediaPlayer 组件 |
| "打开视频失败" | 检查视频文件路径，确保文件存在于 StreamingAssets 中 |
| 视频未显示 | 分配 DisplayUGUI 组件并确保 UI Canvas 配置正确 |
| 按钮无响应 | 确认按钮已分配且在层级中未被禁用 |

## 性能注意事项

- 在加载画面期间初始化视频以获得更平滑的播放体验
- 避免同时加载过多视频
- 使用压缩视频格式以提高性能
- 考虑平台特定的视频编解码器（如 H.264、VP8 等）

## 许可证

本脚本按原样提供以供在 Unity 项目中使用。可根据 MIT 许可进行自由修改和分发。

## 更新日志

### 版本 1.0.0 (2026-01-19)
- 初始发布
- 支持多个视频组
- 播放/暂停/停止 控制
- 带校验的异步加载
- 完整的错误处理
- 完整的 API 文档

## 支持

如有问题、疑问或功能请求，请在仓库中打开 Issue。

---

**注意：** 本脚本旨在作为视频播放管理的辅助工具。有关 AVPro Video 的高级功能，请参考 [官方文档](https://www.renderheads.com/content/documentation/)。
